...
function tagcloudHelper(tags, options) {
  ...
  tags.forEach(tag => {
    ...
    if(tag.name.indexOf("十八个闪耀瞬间") != -1 && !s183){
        s183 = true
        result.push(
      `<a href="/%E5%8D%81%E5%85%AB%E4%B8%AA%E9%97%AA%E8%80%80%E7%9E%AC%E9%97%B4/" style="${style_normal}"${className ? ` class="${className}-${Math.round(ratio_normal * level)} snh_group"` : ''}>十八个闪耀瞬间${showCount ? `<span class="${countClassName}">17</span>` : ''}</a>`
      );
    }
